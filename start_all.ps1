...
if ($hasOllamaInstalled) {
    ...
    Write-Host "[INFO] Ollama non present sur cette machine (fallback RAG distant actif)." -ForegroundColor DarkGray
}
...
if ($UseWindowsTerminal -and (Get-Command wt.exe -ErrorAction SilentlyContinue)) {
    ...
    Write-Host "[1/3] Lancement du Backend RepoScroller (FastAPI :$Port)..." -ForegroundColor Cyan
    Start-Process -FilePath "cmd.exe" -WorkingDirectory $devPath -ArgumentList "/k", "start_backend.bat"
    Start-Sleep -Seconds 2

    # 2. Console Scanner Parallele
    if ($StartScanner) {
        Write-Host "[2/3] Lancement du Scanner Parallele ($Workers threads)..." -ForegroundColor Cyan
    ...
    if ($StartWatcher) {
        Write-Host "[3/3] Lancement du Watcher Daemon continu..." -ForegroundColor Cyan
        Start-Process -FilePath "cmd.exe" -WorkingDirectory $devPath -ArgumentList "/k", "start_watcher.bat"
        Start-Sleep -Seconds 1
    }

    # 4. Console Ollama (si necessaire)
    if ($needStartOllama) {
...
Write-Host "   3. Diagnostics & Telemetrie    : http://127.0.0.1:$Port/api/v1/diagnostics/health" -ForegroundColor Gray
Write-Host "   4. Console Diagnostic Flottante: Integree au Dashboard Web" -ForegroundColor Gray
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host ""